...
echo "Building Docker image with tag: ${REGISTRY_URL}/${REPO_NAME}:${VERSION}"
# Build the Docker image
docker image build --no-cache -t "${REGISTRY_URL}/${REPO_NAME}:${VERSION}" . || { logger "Error building Docker image"; exit 1; }
